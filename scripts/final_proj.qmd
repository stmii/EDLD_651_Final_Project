...
Maritza Note: I don't like this plot(the one below) but need to play with the data
```{r}
# Line plot:Retention trends by sector
overall_persistence_selected |>
  ggplot(aes(x = entering_cohort_year, y = second_fall_retention, color = sector)) +
  geom_line(size = 0.25) +
  labs(
...
```
Retention rates show consistent differences across sectors. Private institutions generally maintain higher second-fall retention compared to public institutions, though both sectors demonstrate gradual improvement over time.
